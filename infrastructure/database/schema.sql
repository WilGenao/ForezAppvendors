-- ============================================================
-- ForexBot Marketplace — Schema Completo de Base de Datos
-- Decisión: PostgreSQL 15 con extensiones uuid-ossp y pg_trgm
-- ============================================================
...
-- ============================================================
-- TIPOS ENUMERADOS
-- Justificación: más rápidos que VARCHAR + CHECK, auto-documentan
-- el dominio. En PG >= 12 ALTER TYPE es transaccional.
-- ============================================================
...
-- ============================================================
-- TABLA: users
-- Soft delete: compliance + audit trail + referencias en payments.
-- Unique parcial en email permite re-registro después de borrado.
-- ============================================================
...
-- ============================================================
-- TABLA: user_roles
-- Separada de users: un user puede ser buyer Y seller.
-- revoked_at permite historial de roles sin borrar registros.
-- ============================================================
...
-- FK a users: 1:1 opcional (no todo user es seller).
-- stripe_account_id: Stripe Connect — pago directo a vendedor.
-- ============================================================
...
CREATE INDEX idx_seller_profiles_verified ON seller_profiles(is_verified_seller) WHERE is_verified_seller = TRUE;

-- ============================================================
-- TABLA: buyer_profiles
-- Separada de seller_profiles: datos distintos, queries distintas.
-- ============================================================
...
-- Hard delete NUNCA. Esto es compliance.
-- Un seller puede tener múltiples intentos, solo 1 aprobado.
-- ============================================================
...
-- ============================================================
-- TABLA: categories
-- Árbol de categorías con self-join para subcategorías.
-- ============================================================
...
-- ============================================================
-- TABLA: bots
-- Entidad central. bot_status controla visibilidad.
-- Soft delete: preserva historial de compras y reviews.
-- ============================================================
...
CREATE INDEX idx_bots_is_verified ON bots(is_verified) WHERE deleted_at IS NULL AND is_verified = TRUE;
CREATE INDEX idx_bots_name_trgm   ON bots USING gin(name gin_trgm_ops) WHERE deleted_at IS NULL;

-- ============================================================
-- TABLA: bot_versions
-- Versionado semántico de los EAs. La versión activa del bot
-- se referencia desde bot_listings. No se borran versiones —
-- los compradores necesitan saber qué versión compraron.
-- ============================================================
...
CREATE TABLE bot_versions (
    ...
    file_hash       VARCHAR(64) NOT NULL, -- SHA-256 del .ex4/.ex5
...
CREATE INDEX idx_bot_versions_is_current ON bot_versions(bot_id, is_current) WHERE is_current = TRUE;

-- ============================================================
-- TABLA: bot_listings
-- Desacopla el "producto" (bot) del "precio" (listing).
-- Un bot puede tener múltiples listings (mensual + anual).
-- price_cents en centavos: nunca float para dinero.
-- ============================================================
...
CREATE TABLE bot_listings (
    ...
    max_licenses            INTEGER,       -- NULL = ilimitado
...
CREATE INDEX idx_bot_listings_category  ON bot_listings(category_id) WHERE deleted_at IS NULL;

-- ============================================================
-- TABLA: subscriptions
-- Mapea 1:1 con Stripe Subscription cuando es recurrente.
-- Para one_time, stripe_subscription_id es NULL.
-- ============================================================
...
-- ============================================================
-- TABLA: payments
-- Hard delete NUNCA. Registro contable inmutable.
-- amount_cents: siempre entero en centavos.
-- ============================================================
...
-- ============================================================
-- TABLA: stripe_events
-- Idempotencia de webhooks: si llega 2 veces el mismo event_id,
-- el UNIQUE constraint rechaza el duplicado silenciosamente.
-- processed_at NULL = aún en cola de procesamiento.
-- ============================================================
...
-- ============================================================
-- TABLA: trade_history
-- La más grande del sistema. Índice compuesto en (bot_id, opened_at)
-- es crítico — sin él, cualquier query de métricas escanea la tabla.
-- Particionamiento por bot_id + rango de fecha cuando >5M rows.
-- ============================================================
...
-- CRÍTICO: estos dos índices son los que salvan las queries de analytics
CREATE INDEX idx_trade_history_bot_time ON trade_history(bot_id, opened_at DESC);
CREATE INDEX idx_trade_history_symbol   ON trade_history(bot_id, symbol);
...
-- Snapshots periódicos de métricas calculadas (diario/semanal).
-- Evita recalcular desde trade_history en cada request.
-- snapshot_date + bot_id es unique: un snapshot por bot por día.
-- ============================================================
...
-- ============================================================
-- TABLA: anomaly_flags
-- Registro de anomalías detectadas por el Analytics Engine.
-- resolved_at: cuando el admin o seller resuelve la anomalía.
-- ============================================================
...
CREATE INDEX idx_anomaly_flags_severity ON anomaly_flags(severity) WHERE is_active = TRUE;

-- ============================================================
-- TABLA: licenses
-- Una licencia = derecho de uso de un bot en N cuentas MT.
-- FK a subscription: si subscription se cancela, licencia expira.
-- Hardware ID (hwid): fingerprint de la cuenta MT del comprador.
-- ============================================================
...
CREATE TABLE licenses (
    ...
    hwid_hash           TEXT[],        -- hashes de hardware IDs registrados
...
CREATE INDEX idx_licenses_bot_id      ON licenses(bot_id);
CREATE INDEX idx_licenses_sub_id      ON licenses(subscription_id);

-- ============================================================
-- TABLA: license_validations
-- Log de cada llamada de validación desde MT4/5.
-- Volumen alto: puede llegar 1 validación cada X minutos por bot.
-- No tiene FK a un "user" porque la llama el EA directamente.
-- Purge automático cada 90 días (cron job).
-- ============================================================
...
-- BRIN es ideal para series de tiempo append-only (muy eficiente en espacio)
...
-- ============================================================
-- TABLA: reviews
-- soft delete: reviews borradas deben seguir en el conteo histórico
-- con "deleted" state para no inflar/desinflar ratings retroactivamente.
-- ============================================================
...
CREATE INDEX idx_reviews_rating   ON reviews(bot_id, rating) WHERE deleted_at IS NULL;

-- ============================================================
-- TABLA: reputation_scores
-- Score agregado por bot, calculado por Analytics Engine.
-- Separado de bots para no lockear la tabla principal en updates.
-- ============================================================
...
-- user_id puede ser NULL (acciones de sistema).
-- changes: diff JSON {before: {}, after: {}} para updates.
-- ============================================================
...
-- Índice parcial en entity para búsqueda por entidad específica
...
-- ============================================================
-- TABLA: api_keys
-- Para vendedores: autenticar el EA de MT4/5 al enviar historial.
-- key_hash: nunca guardamos la clave en texto plano.
-- key_prefix: primeros 8 chars para mostrar al usuario "sk_live_abcd..."
-- ============================================================
...
-- Vista: bots activos con su último snapshot de performance
CREATE VIEW v_active_bot_listings AS
SELECT
    bl.id            AS listing_id,
    bl.listing_type,
    bl.price_cents,
