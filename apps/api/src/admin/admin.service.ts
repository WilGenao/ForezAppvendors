...
export class AdminService {
  ...
  async listBotsForReview(status?: string, page = 1, limit = 20) {
    ...
    const [rows, countResult] = await Promise.all([
      ...
      this.dataSource.query(
        `SELECT COUNT(*) FROM bots WHERE deleted_at IS NULL ${whereStatus}`,
      ),
    ]);
  ...
  async updateBotStatus(
    ...
    const [bot] = await this.dataSource.query(
      `SELECT id, name, status FROM bots WHERE id = $1 AND deleted_at IS NULL`,
      [botId],
    );
  ...
  async listUsers(search?: string, page = 1, limit = 20) {
    ...
    const [rows, countResult] = await Promise.all([
      ...
      this.dataSource.query(
        `SELECT COUNT(*) FROM users WHERE deleted_at IS NULL ${search ? `AND email ILIKE $1` : ''}`,
        search ? [`%${search}%`] : [],
      ),
    ]);
  ...
  async setUserStatus(userId: string, status: string, adminId: string) {
    ...
    await this.dataSource.query(
      ...
      [adminId, userId, JSON.stringify({ before: { status: user.status }, after: { status } })],
    );
