...
export class ReviewsService {
  ...
  async create(userId: string, dto: CreateReviewDto) {
    // Solo pueden opinar compradores verificados
    ...
    if (!purchase) throw new ForbiddenException('You must have purchased this bot to leave a review');
    ...
    // Actualizar avg_rating en bots
    await this.dataSource.query(
      `UPDATE bots SET avg_rating = (SELECT AVG(rating) FROM reviews WHERE bot_id = $1 AND deleted_at IS NULL), total_reviews = (SELECT COUNT(*) FROM reviews WHERE bot_id = $1 AND deleted_at IS NULL) WHERE id = $1`,
      [dto.botId],
    );
