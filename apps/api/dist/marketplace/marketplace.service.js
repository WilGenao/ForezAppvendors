...
let MarketplaceService = MarketplaceService_1 = class MarketplaceService {
    ...
    async getBotDetails(slug) {
       ...
       GROUP BY b.id, sp.display_name, sp.is_verified_seller, sp.avg_rating, ps.sharpe_ratio, ps.win_rate, ps.max_drawdown_pct, ps.profit_factor, ps.total_trades, ps.expectancy_usd, ps.calmar_ratio, ps.sortino_ratio`, [slug]);
        if (!bot)
