...
let SellerService = SellerService_1 = class SellerService {
    ...
    async getRecentSales(userId, page = 1, limit = 20) {
        ...
        const sales = await this.dataSource.query(`SELECT
         p.id,
         p.amount_cents,
    ...
    async getStripeOnboardingUrl(userId) {
        const sellerProfile = await this.getSellerProfile(userId);
        ...
        if (!accountId) {
            ...
            await this.dataSource.query(`UPDATE seller_profiles SET stripe_account_id = $1 WHERE id = $2`, [accountId, sellerProfile.id]);
        }
    ...
    async getSellerProfile(userId) {
        const [profile] = await this.dataSource.query(`SELECT id, user_id, stripe_account_id, stripe_onboarding_done, display_name
       FROM seller_profiles WHERE user_id = $1 LIMIT 1`, [userId]);
        if (!profile)
            throw new common_1.ForbiddenException('Seller profile not found');
        return profile;
    }
    async getRevenueStats(sellerId) {
       ...
       JOIN bot_listings bl ON bl.id = s.bot_listing_id
       JOIN bots b ON b.id = bl.bot_id
       WHERE b.seller_id = $1 AND p.status = 'succeeded'`, [sellerId]);
        return stats;
    }
    async getBotStats(sellerId) {
       ...
       ORDER BY active_subs DESC`, [sellerId]);
    }
    async getMonthlySales(sellerId) {
    ...
    async getRefunds(sellerId) {
       ...
       JOIN bot_listings bl ON bl.id = s.bot_listing_id
       JOIN bots b ON b.id = bl.bot_id
       WHERE b.seller_id = $1 AND p.status = 'refunded'`, [sellerId]);
        return {
