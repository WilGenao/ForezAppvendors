...
let ReviewsService = ReviewsService_1 = class ReviewsService {
    ...
    async create(userId, dto) {
       ...
       VALUES ($1, $2, $3, $4, $5, $6, true) RETURNING id`, [dto.botId, userId, purchase.id, dto.rating, dto.title, dto.body]);
        await this.dataSource.query(`UPDATE bots SET avg_rating = (SELECT AVG(rating) FROM reviews WHERE bot_id = $1 AND deleted_at IS NULL), total_reviews = (SELECT COUNT(*) FROM reviews WHERE bot_id = $1 AND deleted_at IS NULL) WHERE id = $1`, [dto.botId]);
        return { id: review.id, message: 'Review submitted successfully' };
    }
    async listForBot(botId, page = 1, limit = 10) {
        const offset = (page - 1) * limit;
        const reviews = await this.dataSource.query(`SELECT r.id, r.rating, r.title, r.body, r.is_verified_purchase, r.helpful_count, r.created_at,
              u.email as reviewer_email
       FROM reviews r
       JOIN users u ON u.id = r.user_id
       WHERE r.bot_id = $1 AND r.deleted_at IS NULL AND r.is_moderated = true
