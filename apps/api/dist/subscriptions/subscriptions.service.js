...
let SubscriptionsService = SubscriptionsService_1 = class SubscriptionsService {
    constructor(dataSource, config) {
        ...
        this.stripe = new stripe_1.default(config.getOrThrow('STRIPE_SECRET_KEY'), { apiVersion: '2024-04-10' });
    }
    async getBuyerSubscriptions(userId) {
       ...
       ORDER BY
         ...
         s.created_at DESC`, [userId]);
        return rows;
    }
    async cancelSubscription(userId, subscriptionId) {
        const [sub] = await this.dataSource.query(`SELECT id, user_id, stripe_subscription_id, status FROM subscriptions WHERE id = $1`, [subscriptionId]);
        if (!sub)
            throw new common_1.NotFoundException('Subscription not found');
        if (sub.user_id !== userId)
            throw new common_1.ForbiddenException('Not your subscription');
        if (sub.status === 'canceled')
            throw new common_1.ForbiddenException('Already canceled');
        if (sub.stripe_subscription_id) {
            await this.stripe.subscriptions.update(sub.stripe_subscription_id, {
                cancel_at_period_end: true,
            });
        }
        await this.dataSource.query(`UPDATE subscriptions SET canceled_at = NOW(), updated_at = NOW() WHERE id = $1`, [subscriptionId]);
    ...
    async reactivateSubscription(userId, subscriptionId) {
        const [sub] = await this.dataSource.query(`SELECT id, user_id, stripe_subscription_id, status, canceled_at FROM subscriptions WHERE id = $1`, [subscriptionId]);
        if (!sub)
            throw new common_1.NotFoundException('Subscription not found');
        if (sub.user_id !== userId)
            throw new common_1.ForbiddenException('Not your subscription');
        if (sub.status === 'canceled' && !sub.stripe_subscription_id) {
            throw new common_1.ForbiddenException('Subscription has fully expired and cannot be reactivated. Please subscribe again.');
        }
        if (sub.stripe_subscription_id) {
            await this.stripe.subscriptions.update(sub.stripe_subscription_id, {
        ...
        await this.dataSource.query(`UPDATE subscriptions SET canceled_at = NULL, updated_at = NOW() WHERE id = $1`, [subscriptionId]);
        this.logger.log({ msg: 'Subscription reactivated', subscriptionId, userId });
        return { message: 'Subscription reactivated successfully.' };
    }
    async getBillingPortalUrl(userId) {
        const [row] = await this.dataSource.query(`SELECT stripe_customer_id FROM buyer_profiles WHERE user_id = $1`, [userId]);
        if (!row?.stripe_customer_id) {
            throw new common_1.NotFoundException('No billing account found. Make a purchase first.');
        }
