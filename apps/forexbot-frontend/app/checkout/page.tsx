...
const MOCK_BOT = {
  id: '1', name: 'EuroScalper.Pro', seller: 'AlgoTrader Labs',
  platform: 'MT5', pair: 'EURUSD', price: 89, verified: true,
  wr: 68.4, sharpe: 2.31, dd: -4.2,
};
                        ...
                        return (
                          <button key={p.id} onClick={() => setPlan(p.id)}
                            className={`w-full text-left p-4 border transition-colors ${
                              plan === p.id ? 'border-mt-blue bg-mt-blue/5' : 'border-border hover:border-[#333]'
                            }`}>
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  <div className="flex items-center gap-2">
                                    ...
                                    {p.badge && <span className="font-mono text-[9px] bg-mt-green/10 border border-mt-green/30 text-mt-green px-1.5 py-0.5">{p.badge}</span>}
                                  </div>
                              ...
                              <div className="text-right">
                                ...
                                {p.multiplier > 1 && <div className="font-mono text-[10px] text-muted line-through">${(basePrice * p.multiplier).toFixed(2)}</div>}
                              </div>
                            </div>
                          </button>
                        );
                ...
                <div className="panel">
                  ...
                  <div className="p-4 space-y-4">
                    ...
                    <div className="flex items-start gap-2 p-3 bg-mt-bg border border-border">
                      <input type="checkbox" checked={agreed} onChange={e => setAgreed(e.target.checked)} className="mt-0.5 accent-mt-blue" />
                      <span className="font-mono text-[10px] text-muted leading-relaxed">
                        I agree to the <Link href="#" className="text-mt-blue hover:underline">Terms of Service</Link> and <Link href="#" className="text-mt-blue hover:underline">Subscription Policy</Link>. License bound to MT account {mtAccount} on {mtPlatform}.
                      </span>
                    </div>
                    {error && (
                      <div className="flex items-center gap-2 text-mt-red">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button onClick={handleSubmitPayment} disabled={loading}
                        ...
                        {loading ? <><span className="animate-spin">◌</span> PROCESSING...</> : <><Lock className="w-3 h-3" /> PAY_NOW(${total})</>}
                      </button>
                    </div>
                  </div>
                </div>
              ...
              {step === 'success' && (
                <div className="panel p-8 text-center space-y-4">
                  ...
                  <div className="flex gap-2 justify-center pt-2">
                    <Link href="/dashboard/buyer" className="font-mono text-xs bg-mt-blue hover:bg-blue-500 text-white px-5 py-2 transition-colors">
                      VIEW_LICENSES() →
                    </Link>
                    <Link href="/marketplace" className="font-mono text-xs border border-border text-muted hover:text-white px-5 py-2 transition-colors">
                      BROWSE_MORE()
                    </Link>
                  </div>
                </div>
